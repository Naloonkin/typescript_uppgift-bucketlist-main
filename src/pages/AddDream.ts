...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const textInput = document.getElementById("dream") as HTMLInputElement;
    const themeSelect = document.getElementById("dream-select") as HTMLSelectElement;

    const textError = document.getElementById("dream-error-message") as HTMLParagraphElement;
    ...
    form.addEventListener("submit", (e) => { // När man klickar på submit (Lägg till) körs denna funktion
        ...
        if (!valid) return; // Om någon av fälten var tomma köra inte funktionen och inget sparas
